...
const { canModifyQueue } = require("../util/infs");
...
module.exports = {
  async play(song, message) {
    ...
    let messageid = await db.fetch(`queue_lastmessageid`)
    if(!messageid){
    } else{
      try{
      let fetched = await queue.textChannel.messages.fetch(messageid)
      fetched.delete();
      } catch(e){}
    }
    try {
     ...
     .setDescription(`Playing [${song.title}](${song.url})[${message.author}]`);
      ...
      await playingMessage.react("🔁");
      await playingMessage.react("⏹");
      db.set(`queue_lastmessageid`, `${playingMessage.id}`)
    } catch (error) {
      console.error(error);
    }
